...
\memberIII{Dr. Chris Mayhorn}   % unnecessary if committeesize=3
...
%Research Theme:
%
%Privacy encompass both social and technical aspects. However, most of the traditional works have approached privacy from a technical standpoint. I tackle the science of privacy from a sociotechnical viewpoint.
%
%Our actions and interactions in a society are not driven solely by individual needs. Instead, we adapt our behavior considering the needs of others, e.g., by being courteous and lending a helping hand. Such acts, even if inconvenient at times, deliver a pleasant social experience. The theme of my research is engineering socially intelligent and privacy-aware software applications, and I recognize social norms and social context as important factors that influence the working of such applications.
%
%Objective:
%
%The key objective of my research are: (1) to engineer applications such that they deliver a pleasant social experience relative to that society, as opposed to individual user experiences, and yet preserve their privacy, and (2) to make engineering of such applications efficient and effective for developers.
%
%To achieve these objectives, in my research I apply techniques from conceptual modeling, argumentation, and crowdsourcing. I have emphasized rigorous evaluation of the proposed technique by empirical research, and have conducted several studies including human subject studies and simulation experiments.
%
%Introductory Example:
%
%Consider a ringer manager as a socially intelligent application installed on Alice's phone. The ringer manager decides the appropriate ringer modes (e.g., loud or silent) for incoming calls.
%
%Suppose Bob, Alice's friend, makes a casual call to Alice when she is in an important meeting with Charlie and Dave. If Alice's phone ring loud, privacy implications may follow. A loud ring intrudes upon Alice's and other meeting attendees' privacy in that the call violates their reasonable expectation to be left alone (intrusion). Further, the attendees may scowl on Alice (disapprobation). If Alice answers the call, those overhearing Alice and Bob's conversation can gain knowledge about her and her interlocutor (information leak).
%
%Suppose that Bob is in a car accident and calls Alice for assistance. Bob's ringer manager can communicate the urgency to Alice's ringer manager, which can then sets her phone to ring loud. It can also notify Charlie and Dave about the situation. Should Alice's phone stay silent for Bob's urgent call, Bob's trust for Alice may reduce, affecting their social relationship. Instead, if the phone rings loud and Alice communicates a rationale to Dave and Charlie, presumably, they would not frown at her.
%
%Challenges:
%
%These examples demonstrate the nontrivial decisions a socially intelligent must make and the implications those decisions have on the stakeholders' social experience and privacy. These nuances prompt us to investigate the research question: How can we engineer applications such that it delivers a social experience but respects its stakeholders' privacy?
%
%Three key challenges in engineering a socially intelligent application to deliver a social experience are understanding (1) what constitutes social experience; (2) how an application's actions influence the social experience and privacy for each stakeholder; and (3) how an application's actions evolve when it is put to use in a variety of social contexts.
%
%Existing agent-oriented software engineering (AOSE) methods provide a good starting point for addressing the first challenge. But these methods provide little guidance on capturing how an agent's actions and interactions influence each stakeholder involved (second challenge). Also, these methods provide design-time constructs to model an agent, but fall short in modeling social interactions that support agents to adapt to evolving social contexts at run time (third challenge).
%
%Contribution and Novelty:
%
%To address the aforesaid challenges, I propose Arnor, a systematic method enabling the development of privacy-aware socially intelligent applications via social constructs. Arnor facilitates developers in modeling stakeholders' social expectations and, how an application's actions influence those expectations, hereby enabling applications that deliver a rich social experience. Arnor employs Singh's model of (social) norms to capture social requirements, and incorporates argumentation constructs for sharing a decision rationale. Testing a social application's adaptability in all possible social contexts would be infeasible. To overcome this challenge, Arnor incorporates a simulation testbed. Seeded with crowdsourced data, Arnor's testbed enables designers to test a social application's runtime adaptability. I rigorously evaluate Arnor via two studies: (1) a multiphase developer study in which developers engineer a social application, and (2) a set of adaptability studies in which we simulate the adaptability of SIPAs developed in the first study in a variety of social contexts.
%
%Arnor goes beyond existing AOSE methods by assisting developers to incorporate social norms and reason about how those norms influence social experience. In spirit, Arnor is a hybrid method in that it addresses the problem of engineering SIPAs combining top-down (via modeling) and bottomup (via experience or social learning) styles.
%
%Arnor Method:
%
%Arnor is a four-step method that guides developers to systematically engineer a social application's social experience. Arnor's steps include modeling: goals, social contexts, social expectations, and social experience. Goal modeling enables a social application to be aware of its stakeholders, their goals, and relevant plans. Context modeling includes identifying social contexts in which stakeholders of a social application interact. The context plays a decisive role in which goals to bring about or which plans to execute during inconsistencies. Social expectation modeling includes identifying norms and sanctions that govern stakeholders' goals and plans. Social experience modeling includes identifying social application's actions that promote greater social experience, i.e, choosing which plans to execute, which goal states to accomplish, and which norms to satisfy.
%
%Evaluation of Arnor:
%
%To evaluate Arnor against Xipho, an existing AOSE method, I conducted: (1) a developer study in which 30 developers engineer ringer manager social application, and (2) simulation experiments under various adaptation scenarios on the ringer manager social application developed during developer study. I used Xipho as my baseline method to evaluate Arnor, as it is best suited among the existing AOSE methods to engineer such social applications.
%
%In the developer study, I found that developers who follow Arnor produced (1) better models, (2) expended less time and less effort, and (3) felt it is easier to develop a social application, than those who follow Xipho. In the simulation study, I found that applications developed using Arnor provided greater social experience than those developed using Xipho.
%
%Conclusion:
%
%I developed Arnor, a method that provides social constructs to engineer privacy-aware social applications. We demonstrate the method via a ringer manager SIPA. I evaluated Arnor using a developer study and simulation experiments. Compared to Xipho, found that Arnor (1) facilitates faster development of social applications; and (2) yields applications of higher quality, suggesting that Arnor promotes social applications to deliver a rich social experience.
%
%In summary, my research advances the science of privacy by tackling nuanced notions of privacy, including intrusion, disapprobation, and information leakage, in personal agents. In my research, I treat respecting stakeholders' privacy as an inherent aspect of delivering a social experience. I envision socially intelligent applications that (1) adapt to the social contexts of their stakeholders; and (2) act and interact in their best interest (not just the primary stakeholder).
...
%Does a SIPA's ability to reason about shared context help
%the SIPA in learning contextually relevant social norms, and thus
%delivering a rich social experience to its stakeholders?

%\begin{description} % 
%\item[RQ 2a.] Does a SIPA's ability to learn contextually relevant 
...
%
%\section{Modeling Social Intelligence via Norms}
%
%\paragraph{Goal.} Socially intelligent personal agents  are the ones 
%that cater to multiple stakeholders including primary stakeholders, who 
%directly interact with the application, and secondary stakeholders, who 
%are affected by the primary stakeholders' interactions with the agent. 
%The notion of privacy is not just limited to information disclosure 
%without consent, but also includes broader notions of intrusion and 
%disapprobation. The goal is to develop \frameworkA, a norm-based agent-oriented 
%software engineering methodology to engineer privacy-aware personal 
%agents.

%\paragraph{Research questions.} How can we engineer a SIPA such that it delivers a social
%experience but respects its stakeholders' privacy? 
%
...
%
%
%\section{Enhancing Social Experience via Context Sharing}
%
%\paragraph{Goal.} The goal is to develop \frameworkB, a framework that enables 
%SIPAs to share context, and provides ability to reason about shared contexts. 
%
%\paragraph{Research question.} Does a SIPA's ability to reason about shared context help
%the SIPA in learning contextually relevant social norms, and thus
%delivering a rich social experience to its users?
%
%\paragraph{Background.}
%
%\paragraph{Hypotheses.}
...
%
%\section{Enriching Social Experience with Values}
%
...
We execute the ringer manager SIPAs developed in the developer study on a
testbed fabricated to simulate different real-world environments.
...
Figures~\ref{fig:dev-time} and \ref{fig:dev-effort} shows the boxplots
...
\begin{figure}[!htb] \centering
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
\caption[\frameworkA vs.\ Xipho's development time.]{\frameworkA vs.\ Xipho's development time in hours as reported 
...
\begin{figure}[!htb] \centering
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
\caption[\frameworkA vs.\ Xipho's development effort.]{\frameworkA vs.\ Xipho's development effort as reported in the work 
session surveys.}
...
\begin{figure}[!tb]
    ...
    \end{tikzpicture}
    \caption{\frameworkA vs.\ Xipho's norm compliance.}
...
\begin{figure}[!tb]
  ...
  \hspace{2em}
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{\frameworkA vs.\ Xipho's sanction proportion.}
...
% Several methods exist for designing normative systems. Hao
% \etal~\shortcite{Hao-FSE16-Norms+formal} propose a formal method to
% design normative systems. \mps{Doesn't add much to previous sentence}
% Van Riemsdijk {\etal} \shortcite{vanRiemsdijk-AAMAS15-SociallyAdaptive}
% develop a norm compliance framework to design socially adaptive agents.
% Aldewereld \etal~\shortcite{Aldewereld-TAAS16-GroupNorms} present a
% formalism and mechanism to comply with group norms. Ajmeri {\etal}
% \shortcite{Ajmeri-AAMAS17-Arnor} propose Arnor, an agent-oriented
% software engineering method to model social intelligence in personal
% agents. They suggest that personal agents which can understand and learn
% from the intricacies of social norms, deviations and associated
% arguments provide a rich social experience to their users. These works
% provide worthy insights from a SIPA modeling standpoint. \frameworkB is
% novel in the way it helps SIPAs learn social norms by sharing and
% reasoning about deviation contexts.

% Previous works adopt sanctions as a way to promote norm compliance.
% Recent works explore combining norm communication with sanctions to
% promote cooperation \citep{Andrighetto-2013-PunishVoice}. Van Riemsdijk
% {\etal} \shortcite{vanRiemsdijk-AAMAS15-SAEP} emphasize the
% understanding of norm violation as an important challenge for designing
% socially adaptive agents. To understand a deviation, it is necessary to
% understand the context in which a deviation occurs, a problem that
% \frameworkB addresses.

% Other works have addressed the problems of conflicts, compliance, and
% emergence of norms from either a theoretical front or via simulation.
% Sugawara \shortcite{Sugawara-IJCAI11-Emergence} uses reinforcement
% learning to resolve norm conflicts. Morales {\etal}
% \shortcite{Morales-AAMAS15-LiberalNorms} propose a runtime norm
% synthesis approach. Mashayekhi {\etal}
% \shortcite{Mashayekhi-IJCAI16-Silk} propose Silk, a hybrid mechanism for
% norm emergence and conflict resolution in sociotechnical systems.
% Villatoro {\etal} \shortcite{Villatoro-TAAS13-Robust} present social
% instruments as a way to assist norm emergence. Yu {\etal}
% \shortcite{Yu-AAMAS13-Emergence} suggest using collective learning
% compared to pairwise learning for norm emergence. \frameworkB differs
% from these works in supporting sharing and reasoning about contextual
% information to facilitate learning of contextually relevant norms.

%Mihaylov {\etal} \citep{Mihaylov-2014-Decentralized} propose 

%Andrighetto {\etal} \shortcite{Andrighetto-2013-PunishVoice}

%Sen and Airiau \citep{Sen-IJCAI07-NormEmergence}

% Villatoro \citep{Villatoro-IJCAI11-SocialInstruments}

%Andrighetto {\etal} \citep{andrighetto-2013-socialnorms}

\subsection{Normative Systems}
%The concept ``norm'' is used in different disciplines and thus has
%variant notions such as social expectations, legal laws, linguistic
%imperatives, etc \citep{Boella2009NormativeSystems}. We adopt the concept
%of social norms, which describe interactions between principals in terms
%of what they ought to be, or reactions to behaviors, including
%attempts to apply sanctions. Thus, social norms regulate the
%interactions of the principals involved. Norms and normative systems are
%gaining increasing interest in the computer science community. Meyer
%\etal \citep{Meyer+Wieringa-93} define normative systems as ``systems in
%the behavior of which norms play a role and which need normative
%concepts in order to be described or specified.'' Normative multiagent
%systems as a research area can be defined as the intersection of
%normative systems and multiagent systems. Norms govern much of our
%social lives, and therefore are considered as a key element of
%artificial agents that are expected to behave comparably to humans
%\citep{Boella2006NormativeSystems}.

%Researchers have endeavored to design normative systems. 
...
%\begin{figure}[!htb] \centering
%\includegraphics[angle=0,width={0.95\columnwidth}]{Chapter-3/fig/precious-model-v2}
%\caption{A conceptual model of the \frameworkB society}
